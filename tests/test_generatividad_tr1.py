...
#
#   C, I, N, R son el resultado agregado de esa clasificación
#   semántica. No son números sin significado.
#
# QUÉ VERIFICA ESTE TEST
#   - Universo operativo (carga, coherencia, IDs, gobierna).
#   - Generación de pares: T = n(n-1)/2.
#   - Clasificación semántica agregada reportada por generatividad():
#       C, I, N, R y C+I=T, N+R=C.
#   - im_vs_theta coherente con N > theta_n.
#   - Capa canónica: 24 / 276 / 183 / 153 / 30 / 93.
...
#
# GOBIERNA
#   gobierna es polimórfico: puede contener IDs de red (declaraciones)
#   y referencias a componentes/módulos/dominios operativos
#   (p. ej. "semantica", "contexto", "calculator", "conteos").
#   Solo se rechazan referencias vacías.
#   Si la referencia no está en ids_universo, no se declara inválida:
#   la superficie pública actual no expone un registro contractual
#   de componentes para auditar ese segundo dominio.
#
# REGLAS
#   - No reimplementar _medir_pares ni la semántica de TR1.
#   - No inventar campos públicos ni traza.
#   - No modificar Engine, CONTENEDOR, THETA_24 ni módulos ajenos.
#   - No modificar declaraciones existentes.
# ===============================================================
...
def test_generatividad_tr1_contract():
    ...
    # gobierna polimórfico:
    #   - si está en ids_universo → referencia de red resuelta
    #   - si no → componente/módulo/dominio: no auditable aquí
    #     (no hay registro público de componentes)
    # Solo se rechazan referencias vacías.
    operativas = []
    for d in axiomas_teoremas:
        gob = d.get("gobierna")
        ...
        for ref in gob:
            ...
            if ref_s in ids_universo:
                continue
            # Fuera del universo de declaraciones.
            # No se declara inválida ni se convierte en falsa
            # referencia de red. No hay registro contractual
            # público de componentes para auditar este dominio.
        operativas.append(d)
